...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket4)
	{
		...
		DDLogInfo(@"theSocket4: %hu", [self localPortFromCFSocket4:theSocket4]);
	}
	...
	if (theSocket6)
	{
		...
		DDLogInfo(@"theSocket6: %hu", [self localPortFromCFSocket6:theSocket6]);
	}
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		DDLogError(@"AsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		DDLogError (@"AsyncSocket %p couldn't attach read stream to run-loop,", self);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		DDLogError (@"AsyncSocket %p couldn't attach write stream to run-loop,", self);
		DDLogError (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		DDLogError (@"AsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
		DDLogError (@"AsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
...
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
			DDLogError (@"AsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		DDLogError(@"AsyncSocket couldn't determine IP version of socket");
...
- (void)emptyQueues
{
    DDLogInfo(@"AsyncSocket empty queues");
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			DDLogError(@"AsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
    ...
    DDLogInfo(@"AsyncSocket stream call back");
    ///注：这里给currentRead赋值是因为后台push过来，是为了处理theCurrentRead为空的情况
    if (theCurrentRead == nil)
    {
        DDLogInfo(@"AsyncSocket read stream call back, create currentRead");
        theCurrentRead = [[AsyncReadPacket alloc] initWithData:nil
                                                  startOffset:0
                                                    maxLength:0
                                                      timeout:-1
                                                   readLength:0
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			DDLogError(@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			DDLogError(@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
	}
}
